...
class UserViewModel {
    ...
    func loadUsersFromCoreData() {
        ...
        let fetchRequest: NSFetchRequest<RecordsDB> = RecordsDB.fetchRequest()
        do {
            if let result = try context.fetch(fetchRequest).first,
               let wrapper = result.records {
    ...
    func saveUsersToCoreData() {
        ...
        do {
            ...
            let usersWrapper = RecordListWrapper(users: self.users)
            let usersDB = try context.fetch(fetchRequest).first ?? RecordsDB(context: context)
            usersDB.records = usersWrapper
    ...
    func clearUsersFromCoreData() {
        let context = CoreDataManager.shared.context
        let fetchRequest: NSFetchRequest<NSFetchRequestResult> = RecordsDB.fetchRequest()
